...
<!--      T100<xsl:value-of select="substring-after(substring-before(base-uri(),'.docx.xml'),'T100')"/>-->
  ...
  <xsl:template match="tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:p">
    ...
    <xsl:variable name="docplace">
      <xsl:value-of select='tei:sanitize(substring-after(//tei:body/tei:p[5], ":"))'/>
    </xsl:variable>
    <xsl:variable name="partix">
      <xsl:value-of
        select='tei:sanitize(substring-before(substring-after(//tei:body/tei:p[8], ":"), "("))'/>
    </xsl:variable>
  ...
  <xsl:template match="tei:hi[@rend]">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message>
          <xsl:text>Unanticipated @rend value : </xsl:text>
